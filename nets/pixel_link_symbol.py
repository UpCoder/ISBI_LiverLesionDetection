...
#             target_shape = target_layer.shape[1:-1] # NHWC
    ...
    def _fuse_feat_layers(self):
        ...
        self.pixel_cls_logits = tf.image.resize_images(self.pixel_cls_logits, [512, 512])
        self.pixel_link_logits = tf.image.resize_images(self.pixel_link_logits, [512, 512])
...
class PixelLinkNetModify(object):
    def __init__(self, inputs, mask_input, is_training):
        self.inputs = inputs
        self.is_training = is_training
    ...
    def _fuse_feat_layers(self):
        ...
        pixel_link_modify_module = TaskNetworkModuleV2(input_tensors, config.num_neighbours * 2,
                                                     [512 / stride, 512 / stride],
                                                     self.arg_scope, name='pixel_link_modify')
        self.pixel_cls_logits = pixel_cls_modify_module.output
        self.pixel_link_logits = pixel_link_modify_module.output
        self.pixel_cls_logits = tf.image.resize_images(self.pixel_cls_logits, [512, 512])
        self.pixel_link_logits = tf.image.resize_images(self.pixel_link_logits, [512, 512])
        if ms_flag:
            self.pixel_cls_logits_ms = pixel_cls_modify_module.different_scale_outputs
        # self.pixel_cls_logits_ms = [tf.image.resize_images(ele, [512, 512]) for ele in self.pixel_cls_logits_ms]
    ...
    def build_cls_loss(self, batch_size, pos_mask, neg_mask, pos_mask_flatten, neg_mask_flatten, n_pos,
                       pos_pixel_weights_flatten, pixel_cls_weights, do_summary, pixel_cls_loss_weight_lambda):
        ...
        with tf.name_scope('pixel_cls_loss'):
            ...
            def has_pos():
                ...
                cur_pixel_cls_weights = pos_pixel_weights_flatten + \
                                        tf.cast(selected_neg_pixel_mask, tf.float32)
                n_neg = tf.cast(tf.reduce_sum(selected_neg_pixel_mask), tf.float32)
            ...
            #             pixel_cls_loss = tf.cond(n_pos > 0, has_pos, no_pos)
            # pixel_cls_loss, pixel_cls_loss_ms = has_pos()
            pixel_cls_loss = has_pos()
    ...
    def build_loss(self, pixel_cls_labels, pixel_cls_weights,
        ...
        pixel_cls_loss, pixel_cls_dice, pixel_cls_dice_loss = self.build_cls_loss(batch_size, pos_mask, neg_mask,
                                                                                  ...
                                                                                  n_pos,
                                                                                  pos_pixel_weights_flatten,
                                                                                  pixel_cls_weights, do_summary,
        ...
        with tf.name_scope('pixel_link_loss'):
            ...
            def has_pos():
                ...
                def get_loss(label):
                    ...
                    link_weights = pixel_link_weights * tf.cast(link_mask, tf.float32)
                    n_links = tf.reduce_sum(link_weights)
                    loss = tf.reduce_sum(pixel_link_loss * link_weights) / (n_links + 1e-5)
                    return loss
