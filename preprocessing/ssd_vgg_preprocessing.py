...
def distorted_bounding_box_crop(image,
    ...
    with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bboxes, xs, ys]):
        ...
        def no_bboxes():
            ...
            w = tf.constant(0.0000000001, dtype=tf.float32)
            h = w
            xmax = xmin + w
            ymax = ymin + h
        ...
        if mask_image is not None:
            cropped_mask_image = tf.slice(mask_image, bbox_begin, bbox_size)
            cropped_mask_image.set_shape([None, None, 3])
        else:
            cropped_mask_image = None
        ...
        return cropped_image, cropped_mask_image, labels, bboxes, xs, ys, distort_bbox
...
def preprocess_for_train(image, labels, bboxes, xs, ys,
                         ...
                         scope='ssd_preprocessing_train', mask_image=None):
    ...
    """
    fast_mode = False
    with tf.name_scope(scope, 'ssd_preprocessing_train', [image, mask_image, labels, bboxes]):
        if image.get_shape().ndims != 3:
            raise ValueError('Input must be of size [height, width, C>0]')
        if mask_image.get_shape().ndims!=3:
            raise ValueError('Input must be of size [height, width, C>0]')
...
#             image, bboxes, xs, ys = tf_image.random_rotate90(image, bboxes, xs, ys)
        ...
        if USE_ROTATION:
            ...
            def rotate():
                return tf_image.random_rotate90(image, bboxes, xs, ys, mask_image)
 
            def no_rotate():
                return image, mask_image, bboxes, xs, ys
             
            image, mask_image, bboxes, xs, ys = tf.cond(tf.less(rnd, config.rotation_prob), rotate, no_rotate)
        ...
        if MAX_EXPAND_SCALE > 1:
            ...
            def expand():
                ...
                return tf_image.resize_image_bboxes_with_crop_or_pad(
                             image, bboxes, xs, ys, target_h, target_w, mask_image)
 
            def no_expand():
                return image, mask_image, bboxes, xs, ys
             
            image, mask_image, bboxes, xs, ys = tf.cond(tf.less(rnd2, config.expand_prob), expand, no_expand)
...
#         tf_summary_image(image, bboxes, 'image_with_bboxes')
        ...
        dst_image = image
        dst_image, mask_image, labels, bboxes, xs, ys, distort_bbox = \
            distorted_bounding_box_crop(image, labels, bboxes, xs, ys,
                                        min_object_covered = MIN_OBJECT_COVERED,
                                        aspect_ratio_range = CROP_ASPECT_RATIO_RANGE, 
                                        area_range = AREA_RANGE, mask_image=mask_image)
        # Resize image to output size.
        dst_image = tf_image.resize_image(dst_image, out_shape,
                                          method=tf.image.ResizeMethod.BILINEAR,
                                          align_corners=False)
        if mask_image is not None:
            mask_image = tf_image.resize_image(mask_image, out_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR,
                                               align_corners=True)
        tf_summary_image(dst_image, bboxes, 'image_shape_distorted')
        if mask_image is not None:
            tf_summary_image(tf.cast(tf.multiply(mask_image, 200), tf.uint8), None, 'mask_image')
        ...
        return image, mask_image, labels, bboxes, xs, ys
...
def preprocess_image(image,
    ...
    if is_training:
        return preprocess_for_train(image, labels, bboxes, xs, ys,
                                    out_shape=out_shape,
                                    data_format=data_format, mask_image=mask_image)
    else:
