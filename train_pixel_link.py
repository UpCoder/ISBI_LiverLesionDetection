...
tf.app.flags.DEFINE_boolean('modify_flag', False, '')
tf.app.flags.DEFINE_boolean('mask_flag', False, '')
...
def create_dataset_batch_queue(dataset, preprocessing_flag=True):
    ...
    with tf.device('/cpu:0'):
        ...
        if FLAGS.mask_flag:
            [image, mask_image, glabel, gbboxes, x1, x2, x3, x4, y1, y2, y3, y4] = provider.get([
                                                             'image',
                                                             'mask_image',
                                                             'object/label',
                                                             'object/bbox',
        ...
        # gxs = tf.transpose(tf.stack([y1, y2, y3, y4]))
        # gys = tf.transpose(tf.stack([x1, x2, x3, x4]))
        ...
        if FLAGS.mask_flag:
            # mask_image = tf.transpose(mask_image, [1, 0, 2])
        ...
        if preprocessing_flag:
            image, mask_image, glabel, gbboxes, gxs, gys = \
                    ssd_vgg_preprocessing.preprocess_image(
                           image, glabel, gbboxes, gxs, gys,
                           out_shape = config.train_image_shape,
                           data_format = config.data_format,
                           use_rotation = config.use_rotation,
                           is_training = True, mask_image=mask_image)
        else:
            from preprocessing import tf_image
            image = tf_image.resize_image(image, config.train_image_shape)
        image = tf.identity(image, 'processed_image')
        if FLAGS.mask_flag is not None:
        ...
        with tf.name_scope(FLAGS.dataset_name + '_batch'):
            if FLAGS.mask_flag is None:
                ...
                b_image, b_mask_image, b_pixel_cls_label, b_pixel_cls_weight, \
                b_pixel_link_label, b_pixel_link_weight = \
                    tf.train.batch(
                        [image, mask_image, pixel_cls_label, pixel_cls_weight,
                         pixel_link_label, pixel_link_weight],
                        batch_size=config.batch_size_per_gpu,
                        num_threads=FLAGS.num_preprocessing_threads,
                        capacity=500)
            ...
            with tf.name_scope(config.clone_scopes[clone_idx]) as clone_scope:
                with tf.device(gpu) as clone_device:
                    if FLAGS.mask_flag is None:
                        ...
                        b_image, b_mask_image, b_pixel_cls_label, b_pixel_cls_weight, b_pixel_link_label,\
                        b_pixel_link_weight = batch_queue.dequeue()
                    # build model and loss
                    if not FLAGS.modify_flag:
                    ...
                    if FLAGS.mask_flag and FLAGS.modify_flag:
                        print(len(losses))
                        assert len(losses) ==  3
                    else:
                        assert len(losses) ==  2
